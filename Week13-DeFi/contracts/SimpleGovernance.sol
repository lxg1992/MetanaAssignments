...
contract SimpleGovernance is ISimpleGovernance {
    ...
    function queueAction(
        ...
        uint128 value,
        bytes calldata data
    ) external override returns (uint256 actionId) {
    ...
    function getActionDelay() external pure override returns (uint256) {
    ...
    function getAction(
        uint256 actionId
    ) external view override returns (GovernanceAction memory) {
        return _actions[actionId];
    }

    function getActionCounter() external view override returns (uint256) {
